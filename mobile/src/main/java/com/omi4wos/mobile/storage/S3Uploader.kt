    ...
    override suspend fun uploadInputText(jsonLine: String, fileName: String): Boolean =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val lineKey = jsonLine.hashCode().toUInt().toString(16)
                val objectKey = "input_text/$dateKey/${fileName.removeSuffix(".jsonl")}_$lineKey.jsonl"

                val bytes = jsonLine.toByteArray(Charsets.UTF_8)
                val metadata = ObjectMetadata().apply {
                ...
                Log.i(TAG, "Uploaded input text to s3://${config.bucket}/$objectKey")
                true
            } catch (e: Exception) {
